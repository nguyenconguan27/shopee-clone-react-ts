...
export default function ProductRating({
  ...
  noneActiveClassname = 'w-3 h-3 fill-current text-gray-300'
}: Props) {
  const handleStar = (order: number) => {
    if (order <= rating) {
      return '100%'
    }
      ...
      {Array(5)
        ...
        .map((_, index) => (
          <div key={index} className='relative'>
            <div className='absolute top-0 left-0 h-full overflow-hidden' style={{ width: handleStar(index + 1) }}>
